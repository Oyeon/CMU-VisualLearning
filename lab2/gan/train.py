...
def get_optimizers_and_schedulers(gen, disc):
    ...
    # 100K iterations.
    ##################################################################
    # Using LambdaLR to linearly decay the learning rate to 0
    total_iters_disc = 500000
    total_iters_gen = 100000

    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(
        optim_discriminator,
        lr_lambda=lambda step: max(0.0, 1 - step / total_iters_disc)
    )
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(
        optim_generator,
        lr_lambda=lambda step: max(0.0, 1 - step / total_iters_gen)
    )
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                # 2. Discriminator output on real images
                discrim_real = disc(train_batch)

                # 3. Discriminator output on fake images (detach to avoid gradient flow to generator)
                ...
                epsilon = torch.rand(train_batch.size(0), 1, 1, 1, device=train_batch.device, requires_grad=True)
            ...
            if iters >= num_iterations:
                break  # Ensure we don't exceed num_iterations
    fid = get_fid(
        gen,
        dataset_name="cub",
        dataset_resolution=32,
