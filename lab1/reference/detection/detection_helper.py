    ...
    for _iter in range(max_iters):
        ...
        if _iter % log_period == 0:
            ...
            with torch.no_grad():
                ...
                visualize_detections(images[0].cpu().permute(1, 2, 0), outputs, save_path=f"train_epoch_{_iter}.png")
            detector.train()            
...
def inference_with_detector(
    ...
    for iter_num, test_batch in enumerate(test_loader):
        ...
        # Debugging: Print shapes and contents of predictions
        ...
        if pred_boxes.shape[0] == 0:
            print("No predictions found, skipping iteration.")
            continue
        ...
        # Debugging: Print valid predictions
        print(f"valid_pred shape: {valid_pred.shape}")
        print(f"pred_boxes (valid) shape: {pred_boxes.shape}")
        print(f"pred_classes (valid) shape: {pred_classes.shape}")
        print(f"pred_scores (valid) shape: {pred_scores.shape}")
        ...
        if output_dir is not None:
            ...
            print(f"Added image {iter_num} to all_images")
    ...
    if output_dir is None:
        if all_images:
            ...
            print("No images were added to all_images.")
    end_t = time.time()
